...
runRender :: Resources -> Render () -> IO ()
runRender = flip evalStateT

type Render = StateT Resources IO
...
data RenderDef = RenderDef { rdShaders :: [(String, GLuint)]
...
newtype Color = Color { unColor :: V4 Float }

newtype UniqueId = UniqueId { unId :: Int } deriving (Enum, Ord, Eq)
